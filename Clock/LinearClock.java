...
public class LinearClock extends AbstractPen {
   ...
   */
   public LinearClock(SketchPad p){
      super(p);
   }
   ...
   and an associated standard pen.
   */
   public LinearClock(SketchPadWindow w){
      this(new SketchPad());
   ...
   public static void drawClock(){
        LinearClock clock = new LinearClock();
        clock.drawTicks();
        ...
        timer.schedule(new TimerTask() {
            ...
            if(ZonedDateTime.now(ZoneId.of("America/Chicago")).getSecond() == ZonedDateTime.now(ZoneId.of("America/Chicago")).getMinute()) {
                p.d(1000);
                clock.drawMHand(244, 5);
            }

            if(ZonedDateTime.now(ZoneId.of("America/Chicago")).getSecond() == ZonedDateTime.now(ZoneId.of("America/Chicago")).getHour() * -30) {
                p.pl("test");
                p.d(1000);
                clock.drawHHand(100, 7);
            }
